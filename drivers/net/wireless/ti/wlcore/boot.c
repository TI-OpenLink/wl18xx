...
static void wl1271_boot_enable_interrupts(struct wl1271 *wl)
{
	...
	wlcore_write_reg(wl, REG_HI_CFG, HI_CFG_DEF_VAL);
}
...
static int wl1271_boot_soft_reset(struct wl1271 *wl)
{
	...
	/* perform soft reset */
	wlcore_write_reg(wl, REG_SLV_SOFT_RESET, ACX_SLV_SOFT_RESET_BIT);

	/* SOFT_RESET is self clearing */
	timeout = jiffies + usecs_to_jiffies(SOFT_RESET_MAX_TIME);
	while (1) {
		boot_data = wlcore_read_reg(wl, REG_SLV_SOFT_RESET);
	...
	/* disable Rx/Tx */
	wlcore_write_reg(wl, REG_ENABLE, 0x0);

	/* disable auto calibration on start*/
	wlcore_write_reg(wl, REG_SPARE_A2, 0xffff);
...
static int wl127x_boot_clk(struct wl1271 *wl)
{
	...
	wlcore_write_reg(wl, REG_PLL_PARAMETERS, clk);

	pause = wlcore_read_reg(wl, REG_PLL_PARAMETERS);

	wl1271_debug(DEBUG_BOOT, "pause1 0x%x", pause);

	pause &= ~(WU_COUNTER_PAUSE_VAL);
	pause |= WU_COUNTER_PAUSE_VAL;
	wlcore_write_reg(wl, REG_WU_COUNTER_PAUSE, pause);
...
int wl1271_load_firmware(struct wl1271 *wl)
{
	...
	wlcore_write_reg(wl, REG_WELP_ARM_COMMAND, WELP_ARM_COMMAND_VAL);
	udelay(500);
	...
	clk = wlcore_read_reg(wl, REG_DRPW_SCRATCH_START);
	...
	wlcore_write_reg(wl, REG_DRPW_SCRATCH_START, clk);
	...
	wlcore_write_reg(wl, REG_EEPROMLESS_IND,
			 wl->rtable[REG_EEPROMLESS_IND]);
	...
	tmp = wlcore_read_reg(wl, REG_SCR_PAD2);
