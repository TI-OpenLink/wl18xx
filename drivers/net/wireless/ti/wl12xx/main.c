...
static int wl12xx_identify_chip(struct wl1271 *wl)
{
	...
	case CHIP_ID_1271_PG10:
		...
		wl->exp.sr_fw_name = WL127X_FW_NAME_SINGLE;
	...
	case CHIP_ID_1271_PG20:
		...
		wl->exp.quirks |= WLCORE_QUIRK_LEGACY_NVS;
		wl->exp.plt_fw_name = WL127X_PLT_FW_NAME;
		wl->exp.sr_fw_name = WL127X_FW_NAME_SINGLE;
		wl->exp.mr_fw_name = WL127X_FW_NAME_MULTI;
		memcpy(&wl->conf.mem, &wl12xx_default_priv_conf.mem_wl127x,
		       sizeof(wl->conf.mem));
	...
	case CHIP_ID_1283_PG20:
		wl1271_debug(DEBUG_BOOT, "chip id 0x%x (1283 PG20)",
			     wl->exp.chip.id);
		wl->exp.plt_fw_name = WL128X_PLT_FW_NAME;
		wl->exp.sr_fw_name = WL128X_FW_NAME_SINGLE;
		wl->exp.mr_fw_name = WL128X_FW_NAME_MULTI;
		break;
...
static void wl12xx_boot_hw_version(struct wl1271 *wl)
{
	u32 fuse;
			    ...
			    struct sk_buff *skb)
{
	...
	if (wl->exp.chip.id == CHIP_ID_1283_PG20) {
		...
		wl1271_debug(DEBUG_TX, "tx_fill_hdr: hlid: %d "
			     "len: %d life: %d mem: %d extra: %d", desc->hlid,
		...
		wl1271_debug(DEBUG_TX, "tx_fill_hdr: pad: %d hlid: %d "
			     "len: %d life: %d mem: %d", pad, desc->hlid,
			     le16_to_cpu(desc->length),
			     le16_to_cpu(desc->life_time),
...
static void wl12xx_tx_delayed_completion(struct wl1271 *wl)
{
