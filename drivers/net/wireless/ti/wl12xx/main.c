...
static int wl12xx_hw_init(struct wl1271 *wl)
{
	...
	if (wl->chip.id == CHIP_ID_1283_PG20) {
		...
		if (ret < 0)
			goto out;
		/*
		 * TODO: this used to be in sta_hw_init but was moved
		 * here to simplify the code (and acoid having to use
		 * one more operation).  This needs to be tested on
		 * wl127x non-STA modes.
		 */
