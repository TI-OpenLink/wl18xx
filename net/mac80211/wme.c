				     ...
				     struct sk_buff *skb)
{
	...
	while (unlikely(local->wmm_acm & BIT(skb->priority)) &&
			(!(local->wmm_admitted & BIT(skb->priority)) ||
					local->wmm_ac_downgraded & BIT(ac))) {

		u32 time_diff = (jiffies_to_msecs(jiffies) - local->downgrade_ts[ac]);
		if ((local->wmm_ac_downgraded & BIT(ac)) &&
...
int ieee80211_delts_request(struct ieee80211_sub_if_data *sdata,
	...
	if (!(local->wmm_acm & BIT(tspec_params->tid)) ||
			!(local->wmm_admitted & BIT(tspec_params->tid)))
		return -EINVAL;
	...
	sdata->local->wmm_admitted &= ~BIT(tspec_params->tid);
	sdata->local->wmm_admitted &= ~BIT(tid_couple[tspec_params->tid]);
	sdata->local->wmm_ac_downgraded &=
...
void ieee80211_set_wme_ac_admitted(struct ieee80211_local *local,
		u8 ac, bool admitted)
{
	switch (ac) {
	case IEEE80211_AC_BK: /* AC_BK */
		if (admitted)
			local->wmm_admitted |= BIT(1) | BIT(2); /* BK/- */
		else
			local->wmm_admitted &= ~(BIT(1) | BIT(2)); /* BK/- */
		break;
	case IEEE80211_AC_VI: /* AC_VI */
		if (admitted)
			local->wmm_admitted |= BIT(4) | BIT(5); /* CL/VI */
		else
			local->wmm_admitted &= ~(BIT(4) | BIT(5)); /* CL/VI */
		break;
	case IEEE80211_AC_VO: /* AC_VO */
		if (admitted)
			local->wmm_admitted |= BIT(6) | BIT(7); /* VO/NC */
		else
			local->wmm_admitted &= ~(BIT(6) | BIT(7)); /* VO/NC */
		break;
	...
	default:
		if (admitted)
			local->wmm_admitted |= BIT(0) | BIT(3); /* BE/EE */
		else
			local->wmm_admitted &= ~(BIT(0) | BIT(3)); /* BE/EE */
		break;
	}
...
wme_rx_action_tspec(struct ieee80211_sub_if_data *sdata,
	...
	case 1:
		...
		if (status_code
			...
			sdata->local->wmm_admitted |= BIT(tid);
			sdata->local->wmm_admitted |= BIT(tid_couple[tid]);
			sdata->local->wmm_ac_downgraded &= ~BIT(ieee802_1d_to_ac[tid]);
			sdata->local->ops->set_wme_medium_time(&sdata->local->hw,
		...
		struct sk_buff *skb)
{
	...
	/*ieee80211_set_wme_ac_admitted(local, ac, false);*/
	local->downgrade_ts[ac] = jiffies_to_msecs(jiffies);
	local->wmm_ac_downgraded |= BIT(ac);
